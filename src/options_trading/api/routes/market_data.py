...
from ..dependencies import get_market_data_service, get_market_data_manager
    ...
    try:
        ...
        for _, row in contracts_df.iterrows():
            contracts.append({
                ...
                "expiry": row.get('expiry').isoformat() if pd.notna(row.get('expiry')) else expiry_date,
                "lot_size": int(row.get('lot_size', 0)) if pd.notna(row.get('lot_size')) else None
            })
...
async def process_historical_data(
    symbol: str,
    expiry_date: Optional[str] = None,              # now optional
    ...
    strikes: Optional[int] = Query(None),
    exchange: str = Query("NSE_EQ"),
    number_of_exp: Optional[int] = Query(settings.latest_expiry),            # last N expiries if expiry_date is None
    market_manager: MarketDataManager = Depends(get_market_data_manager),
):
    """
    ...
    async def _background_job():
        try:
            ...
            logger.exception("Background historical processing failed for %s", symbol,exc_info=True)
...
async def get_historical_results(
    ...
    try:
        ...
        try:
            ...
            logger.debug("Job registry read failed or absent; falling back to disk scan", exc_info=True)
