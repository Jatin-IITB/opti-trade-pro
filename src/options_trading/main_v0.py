...
def create_app() -> FastAPI:
    ...
    app.add_middleware(
        SessionMiddleware,
        secret_key=settings.secret_key or os.getenv("SECRET_KEY", "dev-secret-key"),
    ...
    async def health_check(request: Request):
        return {
            ...
            "services": {
                "auth_service": "initialized" if hasattr(request.app.state, 'auth_service') else "failed",
                "market_data_manager": "initialized" if getattr(request.app.state, 'market_data_manager', None) else "not_initialized",
                "websocket_manager": "initialized" if hasattr(request.app.state, 'websocket_manager') else "failed",
            ...
            if templates:
                return templates.TemplateResponse(
                    "index.html",
                    {
                        ...
                        **user_context,
                        "api_base": "/api/v1",
                        "websocket_url": f"ws://localhost:8000/api/v1/dashboard/ws"
                    }
                )
            else:
                return HTMLResponse(
...
# BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # folder of main_v0.py
# ROOT_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../"))  # go 2 levels up (Options_v2)

# def sample_and_show_heads(parquet_dirs, sample_size=5):
#     if isinstance(parquet_dirs, str):
#         parquet_dirs = [parquet_dirs]  # convert single path into list
    
#     for parquet_dir in parquet_dirs:
#         dir_path = os.path.join(ROOT_DIR, parquet_dir)
...
# # Example usage (relative to Options_v2 folder)
# dirs = [
#     "FINAL_RELIANCE_1minute_14d",
#     "FINAL_RELIANCE_3minute_7d",
#     "FINAL_TATAMOTORS_3minute_7d",
#     "FINAL_ADANIENT_3minute_7d",
#     "FINAL_CDSL_3minute_7d",
#     "FINAL_DRREDDY_3minute_7d",
#     "FINAL_IPCALAB_3minute_7d",
# ]

# sample_and_show_heads(dirs)
