...
from ..models.dashboard import GreeksSnapshot, PositionData, StrategyPerformance
...
class StrategyService:
    ...
    def __init__(
        ...
        backtest_engine: Any | None = None,  # Will be properly typed when implemented
    ...
    async def _compute_performance_for_strategy(self, cfg: dict[str, Any]) -> StrategyPerformance:
        ...
        try:
            ...
            for pos in positions:
                try:
                    ...
                    option_chain = await self.market_data_service.get_option_chain(
                        ...
                        if isinstance(pos["expiry_date"], date)
                        else pos["expiry_date"],
                        strikes_range=0,
                    )
                    ...
                    if pos["option_type"] in ["CE", "CALL"]:
                        matching_options = [
                            o for o in option_chain.call_options if o.strike == pos["strike"]
                        ]
                    else:
                        matching_options = [
                            o for o in option_chain.put_options if o.strike == pos["strike"]
                        ]
            ...
            return StrategyPerformance(
                ...
                risk_level="unknown",
                portfolio_greeks=GreeksSnapshot(
                    delta=Decimal("0"), gamma=Decimal("0"), theta=Decimal("0"), vega=Decimal("0")
                ),
            )
    ...
    async def _update_performance_history(
        self, strategy_id: str, performance: StrategyPerformance
    ) -> None:
        """Update performance history for trend analysis"""
        try:
            ...
            if len(self._performance_history[strategy_id]) > 100:
                self._performance_history[strategy_id] = self._performance_history[strategy_id][
                    -100:
                ]
    ...
    async def run_backtest(
        ...
        try:
            if not self.backtest_engine:
                ...
                return {"error": "Backtesting not available", "strategy_id": strategy_id}
            ...
            raise DataQualityError(f"Backtest failed: {e!s}")
        ...
        try:
            ...
            risk_metrics = {
                ...
                "portfolio_greeks": {
                    ...
                    if strategy.portfolio_greeks.delta
                    else 0,
                    "gamma": float(strategy.portfolio_greeks.gamma)
                    if strategy.portfolio_greeks.gamma
                    else 0,
                    "theta": float(strategy.portfolio_greeks.theta)
                    if strategy.portfolio_greeks.theta
                    else 0,
                    "vega": float(strategy.portfolio_greeks.vega)
                    if strategy.portfolio_greeks.vega
                    else 0,
                },
    ...
    def _calculate_leverage(self, strategy: StrategyPerformance) -> float:
        ...
        total_notional = sum(
            float(abs(pos.current_price * pos.quantity)) for pos in strategy.positions
        )
    ...
    async def create_strategy_config(self, config: dict[str, Any]) -> str:
        ...
        try:
            ...
            raise DataQualityError(f"Strategy creation failed: {e!s}")
    ...
    async def update_strategy_config(self, strategy_id: str, updates: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise DataQualityError(f"Strategy update failed: {e!s}")
