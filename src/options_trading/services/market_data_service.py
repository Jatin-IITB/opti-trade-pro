...
import pandas as pd
import numpy as np
from ..models.dashboard import MarketDataStatus, ConnectionStatus, MarketDataFeed
from ..models.market_data import OptionChain, VolatilitySurface, GreeksSnapshot, OptionData
...
class MarketDataService:
    ...
    async def get_option_chain(
        ...
        *,
        ...
        if not expiry_date:
            ...
            logger.info("No expiry provided - using last-Thursday expiry %s for %s", expiry_date,symbol)

        expiry_is_expired = _is_expired(expiry_date)

        # Decide flow
        if expiry_is_expired:
            raise DataQualityError(f"Requested expiry {expiry_date} is in the past for {symbol}. Use a future expiry or enable expired mode.")        
        else:
            chosen_flow = "live"
        ...
        if chosen_flow == "live":
            ...
            if isinstance(data, dict):
                ...
                spot_val = data.get("underlying_spot_price") or data.get("underlying_price") or data.get("spot_price")
            ...
            for row in strikes_rows:
                ...
                ce = row.get("call_options") or row.get("call_option") or row.get("CE") or row.get("call")
                pe = row.get("put_options") or row.get("put_option") or row.get("PE") or row.get("put")

                def map_leg(leg: dict, opt_type: str) -> Optional[OptionData]:
                    if not leg or not isinstance(leg, dict):
                        return None
                    md = leg.get("market_data") or {}
                    gr = leg.get("option_greeks") or {}

                    last = md.get("ltp") or md.get("close_price") or leg.get("ltp") or leg.get("last_price")
                    bid = md.get("bid_price") or md.get("bid") or md.get("best_bid_price") or leg.get("bid")
                    ask = md.get("ask_price") or md.get("ask") or md.get("best_ask_price") or leg.get("ask")
                    ...
                    bid_dec = to_decimal_safe_np(bid) or (last_dec * Decimal("0.995")).quantize(Decimal("0.01"))
                    ask_dec = to_decimal_safe_np(ask) or (last_dec * Decimal("1.005")).quantize(Decimal("0.01"))
                    iv_dec = to_decimal_safe_np(iv) or Decimal("0.0")

                    try:
                        greeks_obj = GreeksSnapshot(
                            delta=None if _safe_float(gr.get("delta")) is None else Decimal(str(_safe_float(gr.get("delta")))),
                            gamma=None if _safe_float(gr.get("gamma")) is None else Decimal(str(_safe_float(gr.get("gamma")))),
                            theta=None if _safe_float(gr.get("theta")) is None else Decimal(str(_safe_float(gr.get("theta")))),
                            vega=None if _safe_float(gr.get("vega")) is None else Decimal(str(_safe_float(gr.get("vega")))),
                            rho=None if _safe_float(gr.get("rho")) is None else Decimal(str(_safe_float(gr.get("rho")))),
                        )
                    except Exception:
                        # If the model rejects None, replace with zeros (adjust to your model constraints)
                        try:
                            greeks_obj = GreeksSnapshot(delta=Decimal("0"), gamma=Decimal("0"), theta=Decimal("0"), vega=Decimal("0"), rho=Decimal("0"))
                        except Exception:
                            greeks_obj = None
            ...
            if spot_dec and strikes_range and (calls or puts):
                def keep_near_atm(options: List[OptionData]) -> List[OptionData]:
                    return sorted(options, key=lambda x: abs(float(x.strike) - float(spot_dec)))[: max(10, strikes_range * 2)]
    ...
    async def get_volatility_surface(self, symbol: str) -> VolatilitySurface:
        ...
        try:
            ...
            expiries_all = await self._run_blocking(get_expiries, uk, self.market_data_manager.access_token) or []
            ...
            async def process_strike_sample(expiry: str, strike: float, row_for_meta: pd.Series) -> Optional[Dict[str, Any]]:
                async with semaphore:
                    try:
                        ...
                        temp = pd.DataFrame(
                            {"timestamp": [pd.to_datetime(last.get("timestamp", datetime.utcnow()))], "close_option": [last_price], "close_spot": [spot_float]}
                        )
                        temp = await self._run_blocking(compute_iv_in_memory, temp, float(strike), pd.to_datetime(expiry), row_for_meta.get("instrument_type", "CE"))
                        if not temp.empty:
                        ...
                        logger.debug("Skipping strike %s @ %s due to %s", strike, expiry, e, exc_info=True)
                        return None

            # iterate expiries and sample strikes
            for expiry in expiries:
                try:
                    contracts_df = await self.market_data_manager.fetch_contracts_for_expiry(symbol, "NSE_EQ", expiry)
            ...
            surface = VolatilitySurface(symbol=symbol, spot_price=spot_ref, surface_data=surface_rows, timestamp=datetime.utcnow())
        ...
        try:
            if self.market_data_manager:
                ...
                spot_df = await self._maybe_await(
                    getattr(self.market_data_manager.spot_cache, "get_timeseries"),
                    uk,
    ...
    def _get_fallback_option_chain(self, symbol: str, expiry_date: Optional[str]) -> OptionChain:
        if not expiry_date:
            today = date.today()
            cand = self._last_thursday_of_month(today.year, today.month)
