...
from ..models.auth import UserProfile, TokenInfo
...
async def _with_timeout(awaitable: Awaitable, timeout: Optional[float], name: str = "auth"):
...
async def _extract_bearer_from_headers(
    credentials: Optional[HTTPAuthorizationCredentials], authorization: Optional[str]
) -> Optional[str]:
...
def _build_user_dict_from_profile(profile: UserProfile, token: Optional[str] = None) -> Dict[str, Any]:
    return {
...
async def _validate_and_fetch_profile(access_token: str) -> Dict[str, Any]:
    ...
    try:
        async with AuthService() as auth_service:
            ok = await _with_timeout(auth_service.validate_token(access_token), AUTH_VALIDATE_TIMEOUT, "validate_token")
            if not ok:
                raise HTTPException(
            ...
            profile = await _with_timeout(
                auth_service.get_user_profile(access_token), AUTH_VALIDATE_TIMEOUT, "get_user_profile"
            )
...
async def get_current_user(
    ...
    if session_user_id:
        try:
            token_info: Optional[TokenInfo] = await storage.load_token(session_user_id)
            if token_info:
                # compute near-expiry
                ...
                try:
                    if getattr(token_info, "expires_at", None):
                        ...
                        is_near_expiry = (expires_at - now).total_seconds() <= AUTH_NEAR_EXPIRY_SECONDS
                except Exception:
                ...
                if not getattr(token_info, "is_expired", False) and not is_near_expiry and not force_validate:
                ...
                async with AuthService() as auth_service:
                    try:
                        ...
                        if getattr(token_info, "is_expired", False) and getattr(token_info, "refresh_token", None):
                            refreshed: TokenInfo = await _with_timeout(
                                auth_service.refresh_token(token_info.refresh_token, token_info.user_id),
                            ...
                            try:
                                ...
                                return _build_user_dict_from_profile(profile, refreshed.access_token)
                            except Exception:
                                return {"user_id": refreshed.user_id, "token": refreshed.access_token}
                        ...
                        if force_validate or is_near_expiry:
                            ...
                            if ok:
                                try:
                                    ...
                                    return _build_user_dict_from_profile(profile, token_info.access_token)
                                except Exception:
                                    return {"user_id": token_info.user_id, "token": token_info.access_token}
                            # invalid
                        ...
                        logger.warning("AuthService reported error for session user %s: %s", session_user_id, e)
                        # Fallback to trusting stored token when network/DNS issues are transient
                        if token_info and not getattr(token_info, "is_expired", True) and TRUST_STORED_ON_NETWORK_ERROR:
                            logger.warning("Falling back to stored token due to AuthService/network failure.")
                            return {"user_id": token_info.user_id, "token": token_info.access_token}
            ...
            logger.exception("Failed to load token from SecureStorage for session user %s: %s", session_user_id, e)
