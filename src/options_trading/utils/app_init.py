...
async def initialize_app_services(app, user_id: Optional[str] = None, access_token: Optional[str] = None) -> bool:
    """
    ...
    async with app.state.service_init_lock:
        ...
        if not access_token:
            try:
                async with AuthService() as auth:
                    ...
                    access_token = token
                    logger.debug("Resolved access_token via AuthService for user '%s'", user_id or "default")
            except AuthError as e:
                logger.warning("Could not resolve access token for user '%s': %s", user_id or "default", e)
                return False
        ...
        try:
            ...
            if StrategyService:
                app.state.strategy_service = StrategyService(market_data_service=app.state.market_data_service)
            else:
            ...
            for key in ("market_data_manager", "market_data_service", "dashboard_service", "strategy_service"):
