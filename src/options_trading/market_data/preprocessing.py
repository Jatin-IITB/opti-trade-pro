...
def clean_and_merge_option_spot(
    option_df: pd.DataFrame, spot_df: pd.DataFrame, expiry: str, strike: int
) -> pd.DataFrame:
    ...
    expiry_cutoff = pd.Timestamp(datetime.combine(expiry_date, end_time)) - timedelta(
        hours=settings.default_trim_hours
    )
    ...
    spot_df = spot_df.rename(
        columns={"open": "open_spot", "high": "high_spot", "low": "low_spot", "close": "close_spot"}
    )
    ...
    if not validate_option_data(merged, strike, option_df["instrument_type"].iloc[0]):
        logger.warning("Data validation failed - continuing with NaNs")
...
def calculate_greeks(S, K, T, r, sigma, option_type):
    ...
    try:
        ...
        if option_type.lower() in ["ce", "call"]:
            delta = norm_d1
            theta = (
                (-S * norm_pdf_d1 * sigma) / (2 * np.sqrt(T))
...
def append_greeks_in_memory(df: pd.DataFrame, strike, expiry, option_type) -> pd.DataFrame:
    ...
    def calc_greeks_row(row):
        try:
            ...
            sigma = float(row["iv"])
            return pd.Series(
                calculate_greeks(S, strike, T, settings.risk_free_rate, sigma, option_type),
...
def fetch_extended_spot_data(
    df_option, access_token, underlying_key, interval_minutes, fetch_func, cache_func=None
):
    ...
    try:
        if cache_func:
            ...
            df_spot = cache_func(
                underlying_key, from_date, to_date, access_token, interval_minutes, fetch_func
            )
        else:
...
def append_rv_in_memory(
    df, access_token, underlying_key, interval_minutes, fetch_func, window=None, cache_func=None
):
    if window is None:
    ...
    logger.info(
        f"RV calculation complete: GK {gk_coverage:.1f}% coverage, Parkinson {park_coverage:.1f}% coverage"
    )
    return df_merged
