...
def is_expired(expiry_str: str) -> bool:
    try:
        ...
        return True
def get_expiry_slice(expiry: List[str], start: Optional[int] = None, end: Optional[int] = None) -> List[str]:
...
class MarketDataManager:
    ...
    def __init__(self, access_token: str, spot_cache: Optional[MarketDataCache] = None):
    ...
    async def get_underlying_key(self, symbol: str, exchange: str) -> str:
        ...
        return key
    async def fetch_live_contracts_for_expiry(self, symbol: str, exchange: str, expiry: str) -> pd.DataFrame:
    ...
    async def fetch_contracts_for_expiry(self, *args) -> pd.DataFrame:
        if len(args)==2:
            uk,expiry=args
            df = await self._maybe_await(fetch_expired_option_contracts_df, uk, expiry, self.access_token)
            ...
            raise TypeError("fetch_contracts_for_expiry expects (underlying_key, expiry) or (symbol, exchange, expiry)")

    async def save_features_for_expiry(
        self,
        symbol: str,
        exchange: str,
        expiry: Optional[str],                      # allow None if using number_of_exp
        option_interval: str,
        spot_interval: str,
        days_back: int,
        strikes: int,
        out_base: str = "FINAL",
        number_of_exp: Optional[int]=None       # pick last `number_of_exp` expiries if expiry is None
        ...
        """
        logger.debug(f"save_features_for_expiry called: symbol={symbol}, expiry={expiry}, number_of_exp={number_of_exp}, strikes={strikes}")
        ...
        for expiry_str in expiries_to_process:
            ...
            spot_df_short = await self.spot_cache.get_timeseries(
                uk, expiry_str, expiry_str, self.access_token, int(spot_interval),
                fetch_upstox_historical_data, ttl=None, unit="minutes"
            )

            if spot_df_short is None or (hasattr(spot_df_short, "empty") and spot_df_short.empty):
                logger.warning(f"No spot data for {symbol} {exchange} {expiry_str}, skipping expiry")
                continue

            spot_price = float(spot_df_short["close"].iloc[-1])
            logger.debug(f"Spot price for {symbol} {expiry_str}: {spot_price}")

            atm_keys = pick_near_the_money_contracts(contracts_df, spot_price, num_itm=strikes, num_otm=strikes)
            ...
            for key in atm_keys:
                ...
                try:
                    ...
                    expiry_ts = pd.to_datetime(row["expiry"]) if not isinstance(row["expiry"], pd.Timestamp) else row["expiry"]
                    start_date = (expiry_ts - pd.Timedelta(days=days_back)).strftime("%Y-%m-%d")
                    expiry_date_str = expiry_ts.strftime("%Y-%m-%d")

                    # option candles
                    option_df = await self._maybe_await(fetch_option_candles, key, option_interval, start_date, expiry_date_str, self.access_token)
                    ...
                    spot_df_full = await self.spot_cache.get_timeseries(
                        uk, start_date, expiry_date_str, self.access_token, int(spot_interval),
                        fetch_upstox_historical_data, ttl=None, unit="minutes"
                    )
                    ...
                    # merge & compute
                    merged_df = clean_and_merge_option_spot(option_df, spot_df_full, expiry_ts, float(row["strike_price"]))
                    ...
                    merged_df = compute_iv_in_memory(merged_df, float(row["strike_price"]), expiry_ts, instr_type)
                    ...
                    if asyncio.iscoroutinefunction(append_rv_in_memory):
                        ...
                        def cache_callable_sync(uk_, s_, e_, token_, interval_, fetch_func_, ttl=None, unit="minutes"):
                            return asyncio.run(self.spot_cache.get_timeseries(uk_, s_, e_, token_, interval_, fetch_func_, ttl=ttl, unit=unit))
                    ...
                    merged_df = await self._maybe_await(
                        append_rv_in_memory,
                        merged_df, self.access_token, uk, int(spot_interval),
                        fetch_upstox_historical_data,
                        cache_func=cache_callable
                    )

                    # greeks
                    merged_df = await self._maybe_await(append_greeks_in_memory, merged_df, float(row["strike_price"]), expiry_ts, instr_type)

                    # save
                    fname = generate_filename(trading_symbol, expiry_ts.date().isoformat(), instr_type, int(float(row["strike_price"])), extension="parquet")
