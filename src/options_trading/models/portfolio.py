...
class PortfolioSummaryOut(BaseModel):
    ...
    Nullable fields are null when the underlying input is genuinely
    unavailable (no funds call, no live spot yet) rather than defaulting to
    zero — a zero Greek reads as a flat book, which is a different and much
    more dangerous claim than "not yet known".
    """
    ...
    aggregate_greeks: PortfolioGreeksOut | None = None
    greeks_priced: int = 0
    synced: bool = False
...
class PortfolioSyncStatusOut(BaseModel):
    ...
    #: this eventually goes true for every unattended session. The UI must
    #: prompt for re-login rather than showing a silently stale book.
