...
class OAuthConfig(BaseModel):
    ...
    token_url: str = Field(
        "https://api.upstox.com/v2/login/authorization/token", description="Token exchange URL"
    )
    scope: str | None = Field(None, description="OAuth2 scope")
...
class OAuthCallbackRequest(BaseModel):
    ...
    state: str | None = Field(None, description="CSRF protection state parameter")
...
class TokenValidationResponse(BaseModel):
    ...
    expires_at: datetime | None = Field(None, description="Token expiry time")
...
class TokenRefreshResponse(BaseModel):
    ...
    refresh_token: str | None = Field(None, description="New refresh token")
...
class UserProfile(BaseModel):
    ...
    mobile: str | None = Field(None, description="User mobile number")
...
class AuthStatus(BaseModel):
    ...
    user_id: str | None = Field(None, description="Current user ID")
    token_expires_at: datetime | None = Field(None, description="Token expiry time")
    needs_refresh: bool = Field(False, description="Whether token needs refresh")
    last_validated: datetime | None = Field(None, description="Last token validation time")
